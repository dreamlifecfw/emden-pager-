...
// التحقق من الرمز
function checkCode() {
    const correctCode = mode === "send" ? "1889" : "1900";
...
document.getElementById("sendBtn").addEventListener("click", () => {
    ...
    messageBox.innerHTML = `
        <div>${encoded}</div>
        <button class="copy">نسخ الأرقام</button>
    `;

    messages.prepend(messageBox);

    // زر النسخ
    messageBox.querySelector(".copy").addEventListener("click", async () => {

        try {
            await navigator.clipboard.writeText(encoded);

            const copyButton = messageBox.querySelector(".copy");
